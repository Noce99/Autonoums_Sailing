...
import pygame
import torch
from Sailing_Boats_Autopilot.Deep_RL_4_Realistic_Environment.NNs import SimpleCnn
from collections import namedtuple
from Sailing_Boats_Autopilot.Deep_RL_4_Realistic_Environment.Environment import RealisticEnvironment, \
...
BufferElement = namedtuple("BufferElement", ["state", "state_numeric", "action", "reward", "next_state",
                                             "next_state_numeric", "terminated"])
...
class SimpleDQN:
    ...
    def train_on_mini_batch_from_buffer(self, mini_batch_size):
        ...
        mini_batch_state_numeric = torch.concatenate([element.state_numeric[None, :]
                                                      for element in selected_elements], axis=0)
        mini_batch_next_state = torch.concatenate([element.next_state[None, :] for element in selected_elements],
                                                  axis=0)
        mini_batch_next_state_numeric = torch.concatenate([element.next_state_numeric[None, :]
                                                           for element in selected_elements], axis=0)
        ...
        training_nn_result = self.training_nn(mini_batch_state.cuda(), mini_batch_state_numeric.cuda())
        ...
        with torch.no_grad():
            next_states_value = torch.max(self.historical_nn(mini_batch_next_state.cuda(),
                                                             mini_batch_next_state_numeric.cuda()),
                                          dim=1).values.detach()
    ...
    def game_step(self, env, start_tensor=None):
        ...
        state_s_numeric = torch.tensor([env.boat.orientation,
                                        from_start_end_to_direction(env.boat.position, env.target_point)])
        if random.random() < self.epsilon:
            action = random.randint(0, 2)
        else:
            action = int(torch.argmax(self.training_nn(state_s.cuda(), state_s_numeric.cuda())))
        observation, reward, terminated, _, distance = env.step(action)
        state_s_prime = env.observation
        state_s_prime_numeric = torch.tensor([env.boat.orientation,
                                              from_start_end_to_direction(env.boat.position, env.target_point)])
        if terminated:
            self.need_to_start_new_episode = True
        self.add_element_to_buffer(BufferElement(state=state_s,
                                                 state_numeric=state_s_numeric,
                                                 action=action,
                                                 reward=reward,
                                                 next_state=state_s_prime,
                                                 next_state_numeric=state_s_prime_numeric,
    ...
    def start_training_session(self):
        ...
        for _ in my_bar:
            ...
            if step_counter % STEP_BEFORE_SAVE_PLOTS == 0 and step_counter != 0:
                ...
                plt.title("Loss")
                plt.savefig(f"{ABSOLUTE_PATH}/Deep_RL_4_Realistic_Environment/plots/loss.png")
                plt.clf()
                plt.plot(self.episode_steps_history)
                plt.title("Episode Steps")
                plt.savefig(f"{ABSOLUTE_PATH}/Deep_RL_4_Realistic_Environment/plots/steps.png")
                """

            if step_counter % (self.training_steps // 100) == 0:
                now = datetime.now()
                torch.save(self.training_nn.state_dict(), f"{ABSOLUTE_PATH}/Deep_RL_4_Realistic_Environment/models/"
...
def new_train(weight_name=None, render_mode="human"):
    if weight_name is not None:
        weight_path = f"{ABSOLUTE_PATH}/Deep_RL_4_Realistic_Environment/models/{weight_name}"
    else:
        weight_path = None
    dqn = SimpleDQN(training_steps=25000000, fps=20000000, weight_path=weight_path, render_mode=render_mode)
    dqn.start_training_session()
    now = datetime.now()
    torch.save(dqn.training_nn.state_dict(), f"{ABSOLUTE_PATH}/Deep_RL_4_Realistic_Environment/models/"
                                             f"{now.year}_{now.month}_{now.day}_{now.hour}_{now.minute}_"
                                             f"{now.second}.pt")


def visual_evaluation(name, epsilon=0.0):
    dqn = SimpleDQN(training_steps=0, weight_path=f"{ABSOLUTE_PATH}/Deep_RL_4_Realistic_Environment/"
...
if __name__ == "__main__":
    visual_evaluation("working_just_by_wind.pt", epsilon=0.05)
    # new_train(render_mode="no_visual")
