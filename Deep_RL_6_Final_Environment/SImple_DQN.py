...
import pygame
import torch
from Sailing_Boats_Autopilot.Deep_RL_6_Final_Environment.NNs import SimpleCnn
...
from Sailing_Boats_Autopilot.utils import random_location_in_atlantic_ocean, from_start_end_to_direction, lat_converter, \
    lon_converter, random_start_end_race
...
FINISHING_EPSILON = 0.2

START_BUFFER_SIZE = 30000
MAX_BUFFER_SIZE = 30000
STEP_BEFORE_ALIGN_NETWORKS = 2500
STEP_BEFORE_SAVE_PLOTS = 25000
MINI_BATCH_SIZE = 32
GAMMA = 0.99
LEARNING_RATE = 0.001

HISTORY_TMP_LIST_SIZE = 5000

TIME_IN_SECONDS_FOR_STEPS = 1000

TRAINING_STEPS = int(30 * 10**6)

BufferElement = namedtuple("BufferElement", ["state", "state_image", "action", "reward", "next_state",
                                             "next_state_image", "terminated"])


class SimpleDQN:
    def __init__(self, training_steps, weight_path=None, fps=4, render_mode="human"):
        self.size = GRID_PIXELS_SIZE
        self.fps = fps
        ...
        self.reward_history = []
        self.tmp_episode_reward_history = []
    ...
    def train_on_mini_batch_from_buffer(self, mini_batch_size):
        ...
        # noinspection PyArgumentList
        mini_batch_state_image = torch.concatenate([element.state_image[None, :] for element in selected_elements],
                                                   axis=0)
        # noinspection PyArgumentList
        mini_batch_next_state = torch.concatenate([element.next_state[None, :] for element in selected_elements],
                                                  axis=0)
        # noinspection PyArgumentList
        mini_batch_next_state_image = torch.concatenate(
            [element.next_state_image[None, :] for element in selected_elements], axis=0)
        mini_batch_next_state_terminated = torch.tensor([element.terminated for element in selected_elements],
                                                        dtype=torch.bool)
        ...
        training_nn_result = self.training_nn(mini_batch_state.cuda(), mini_batch_state_image.cuda())

        action_values = torch.take(training_nn_result.cpu(), mini_batch_actions)
        with torch.no_grad():
            next_states_value = torch.max(self.historical_nn(mini_batch_next_state.cuda(),
                                                             mini_batch_next_state_image.cuda()), dim=1).values.detach()

        expected_action_values = next_states_value.cpu() * GAMMA + mini_batch_reward
        expected_action_values[mini_batch_next_state_terminated] = mini_batch_reward[mini_batch_next_state_terminated]
    ...
    def game_step(self, env, start_tensor=None):
        ...
        state_s, state_s_image = env.observation

        if random.random() < self.epsilon:
            action = random.randint(0, 2)
        else:
            action = int(torch.argmax(self.training_nn(state_s.cuda(), state_s_image.cuda())))

        observation, reward, terminated, _, distance = env.step(action)
        state_s_prime, state_s_prime_image = env.observation
        ...
        if terminated:
            self.need_to_start_new_episode = True
            self.tmp_episode_reward_history.append(env.episode_reward)
            if len(self.tmp_episode_reward_history) > 30:
                self.episode_reward_history.append(
                    sum(self.tmp_episode_reward_history) / len(self.tmp_episode_reward_history))
                self.tmp_episode_reward_history = []

        self.add_element_to_buffer(BufferElement(state=state_s,
                                                 state_image=state_s_image,
                                                 action=action,
                                                 reward=reward,
                                                 next_state=state_s_prime,
                                                 next_state_image=state_s_prime_image,
    ...
    def start_training_session(self):
        ...
        env = RealisticEnvironment(start_point=random_location_in_atlantic_ocean,
                                   ...
                                   render_mode=self.render_mode, time_in_second_for_step=TIME_IN_SECONDS_FOR_STEPS)
        """

        env = RealisticEnvironment(start_point_target_point_function=random_start_end_race,
                                   render_mode=self.render_mode, time_in_second_for_step=TIME_IN_SECONDS_FOR_STEPS,
                                   fps=self.fps, visualizer_function=None,
                                   random_wind=False)
        step_counter = 0
        ...
        for _ in my_bar:
            self.game_step(env)
            if step_counter % MINI_BATCH_SIZE == 0:
            ...
            if step_counter % (self.training_steps // 100) == 0 and step_counter != 0:
                ...
                plt.title("Loss")
                plt.savefig(f"{ABSOLUTE_PATH}/Deep_RL_6_Final_Environment/plots/loss.png")
                plt.clf()
                plt.plot(self.reward_history)
                plt.title("Reward")
                plt.savefig(f"{ABSOLUTE_PATH}/Deep_RL_6_Final_Environment/plots/reward.png")
                plt.clf()
                plt.plot(self.episode_reward_history)
                plt.title("Episode Reward")
                plt.savefig(f"{ABSOLUTE_PATH}/Deep_RL_6_Final_Environment/plots/episode_reward.png")
                plt.clf()

            if step_counter % (self.training_steps // 100) == 0 and step_counter != 0:
                now = datetime.now()
                torch.save(self.training_nn.state_dict(), f"{ABSOLUTE_PATH}/Deep_RL_6_Final_Environment/models/"
    ...
    def visual_evaluation(self, epsilon=0.0):
        ...
        while True:
            ...
            if random.random() < epsilon:
                ...
                # print(state_s)
                actions_values = self.training_nn(state_s.cuda(), state_s_image.cuda())
...
def new_train(weight_name=None, render_mode="human"):
    if weight_name is not None:
        weight_path = f"{ABSOLUTE_PATH}/Deep_RL_6_Final_Environment/models/{weight_name}"
    else:
        weight_path = None
    dqn = SimpleDQN(training_steps=TRAINING_STEPS, fps=1000000, weight_path=weight_path, render_mode=render_mode)
    dqn.start_training_session()
    now = datetime.now()
    torch.save(dqn.training_nn.state_dict(), f"{ABSOLUTE_PATH}/Deep_RL_6_Final_Environment/models/"
                                             f"{now.year}_{now.month}_{now.day}_{now.hour}_{now.minute}_"
                                             f"{now.second}.pt")


def visual_evaluation(name, epsilon=0.0):
    dqn = SimpleDQN(training_steps=0, weight_path=f"{ABSOLUTE_PATH}/Deep_RL_6_Final_Environment/"
                                                  f"models/{name}", fps=30)
    """
    for param in dqn.training_nn.parameters():
        print(param.data)
    """
...
if __name__ == "__main__":
    # visual_evaluation("2023_9_14_7_7_5.pt", epsilon=0.2)
    new_train(render_mode="no-human", weight_name="senza_vento.pt")
