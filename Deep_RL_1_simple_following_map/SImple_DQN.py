...
EPSILON_ANGULAR_COEFFICIENT = -0.00001
...
class SimpleDQN:
    ...
    def train_on_mini_batch_from_buffer(self, mini_batch_size):
        ...
        mini_batch_actions = torch.tensor([element.action for element in selected_elements]) + \
                             torch.arange(start=0, end=mini_batch_size * 4, step=4, dtype=torch.int)
...
def new_train(size):
    dqn = SimpleDQN(size=size, training_steps=100000, fps=10000)
...
if __name__ == "__main__":
    visual_evaluation(10, "10x10_forte.pt")
    # new_train(5)
