...
class RealisticEnvironment(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        embedded_directions = get_single_directions_embedded(self.boat.orientation, self.boat.position,
                                                             self.target_point)
        distance_angle = angular_distance_degree(self.boat.position, self.target_point)
        embedded_distance = torch.tensor([distance_angle / 180]).cuda()
        self.observation = torch.concatenate([embedded_directions, embedded_distance])
    ...
    def step(self, action):
        ...
        self.boat.integrate(self.constant_wind, self.time_in_second_for_step)
        ...
        #     reward -= max(distance_angle * 100, 500)
            ...
            terminated = False
            if self.boat.angular_distance > distance_angle:
                reward = 0.5/MAX_EPISODE_STEPS
            else:
                reward = -0.5/MAX_EPISODE_STEPS
    ...
    def render_frame(self):
        ...
        text_rect.center = (int(self.window_size - (0.5 * self.window_size - text_rect.width) // 2) -
                            (0.5 * text_rect.width), int(0.5 * self.window_size + 1 / 8 * self.window_size / 2))
        self.window.blit(text, text_rect)

        text = self.font.render(f"wind speed: {wind_speed:.3f} m/s", True, (0, 255, 0), (0, 0, 128))
        text_rect = text.get_rect()
        text_rect.center = (int(self.window_size - (0.5 * self.window_size - text_rect.width) // 2) -
                            (0.5 * text_rect.width), int(0.5 * self.window_size + 2 / 8 * self.window_size / 2))
        self.window.blit(text, text_rect)
        ...
        text_rect = text.get_rect()
        text_rect.center = (int(self.window_size - (0.5 * self.window_size - text_rect.width) // 2) -
                            (0.5 * text_rect.width), int(0.5 * self.window_size + 3 / 8 * self.window_size / 2))
        self.window.blit(text, text_rect)

        angular_distance = self.boat.angular_distance
        text = self.font.render(f"Angular Distance: {angular_distance:.3f} deg",
                                True, (0, 255, 0), (0, 0, 128))
        text_rect = text.get_rect()
        text_rect.center = (int(self.window_size - (0.5 * self.window_size - text_rect.width) // 2) -
                            (0.5 * text_rect.width), int(0.5 * self.window_size + 4 / 8 * self.window_size / 2))
        self.window.blit(text, text_rect)
        ...
        text_rect = text.get_rect()
        text_rect.center = (int(self.window_size - (0.5 * self.window_size - text_rect.width) // 2) -
                            (0.5 * text_rect.width), int(0.5 * self.window_size + 5 / 8 * self.window_size / 2))
        self.window.blit(text, text_rect)

        text = self.font.render(f"{self.observation[:8].int().tolist()}",
                                True, (0, 255, 0), (0, 0, 128))
        text_rect = text.get_rect()
        text_rect.center = (int(self.window_size - (0.5 * self.window_size - text_rect.width) // 2) -
                            (0.5 * text_rect.width), int(0.5 * self.window_size + 6 / 8 * self.window_size / 2))
        self.window.blit(text, text_rect)

        text = self.font.render(f"{self.observation[8:16].int().tolist()}",
                                True, (0, 255, 0), (0, 0, 128))
        text_rect = text.get_rect()
        text_rect.center = (int(self.window_size - (0.5 * self.window_size - text_rect.width) // 2) -
                            (0.5 * text_rect.width), int(0.5 * self.window_size + 7 / 8 * self.window_size / 2))
        self.window.blit(text, text_rect)
